 ...
 * All rights reserved.
 *
 * Date: Sun Jun 30 00:06:54 2013 +0200
 *
 ***
 *
...
PaperScope.inject({
	enumerable: true,
			 ...
			 *
			 *	@example
			 * 	var snapped = Calculation.snap(0.66, 0.2); 
			 *	
			 */
			 ...
			 *
			 *	@example
			 * 	var rad = Calculation.radians(180); 
			 *	
			 */
...
Conversions.instance = new Conversions();
...
var Point = Base.extend({
	...
	lerp: function( arg0, arg1, arg2 ) {
		if(typeof arg1 === 'number') {
			// this.x = lerp(this.x, arg0.x, arg1);
			// this.y = lerp(this.y, arg0.y, arg1);
			// return this;
			return new Point(
				lerp(this.x, arg0.x, arg1),
				lerp(this.y, arg0.y, arg1)
			);
		}
		else {
			return new Point(
				lerp(arg0.x, arg1.x, arg2),
				lerp(arg0.y, arg1.y, arg2)
			);
		}
	},
	...
	norm: function(start, stop) {
		this.x = Calculation.norm(this.x, start.x, stop.x);
		this.y = Calculation.norm(this.y, start.y, stop.y);
		return this;
	},
...
var Rectangle = Base.extend({
	...
	getRadius: function() {
		return this.getSize().radius();
	},

	getDiameter: function() {
		return this.getSize().diameter();
	},
...
}, Base.each(['getBounds', 'getStrokeBounds', 'getHandleBounds', 'getRoughBounds', 'getCircumcircle'],
	function(name) {
		this[name] = function() {
...
var Path = PathItem.extend({
	...
	getCentroid: function() {
		...
		var pt0, pt1;
		for ( var i=0; i<this._segments[0].length-1; ++i ) {
			pt0 = this._segments[i].point;
			pt1 = this._segments[i+1].point;
...
statics: {
	...
	getCircumcircle: function(segments, closed, style, matrix) {
		if( segments.length === 3 ) {
			var p1 = segments[0].point;
			var p2 = segments[1].point;
			var p3 = segments[2].point;
			...
			if( Math.abs(G) < Numerical.EPSILON ) {
				...
				dx = style._item.getCircumcenter().x - minx;
				dy = style._item.getCircumcenter().y - miny;
			}
			else {
				dx = style._item.getCircumcenter().x - p1.x;
				dy = style._item.getCircumcenter().y - p1.y;
			}

			this._radius = Math.sqrt( (dx * dx + dy * dy) );

			return Path.Circle( 
				style._item.getCircumcenter(),
				this._radius
			);
		}
		else {
...
var Color = Base.extend(new function() {
	...
	function _getCSSStyle( className ) {
	...
	function CSStoRGB( className ) {
		var raw, rawSplit, arr, r,g,b,a;
		raw = _getCSSStyle(className);
	...
	return Base.each(types, function(properties, type) {
		...
		_adjust: function(amt) {
			...
			var delta, remainder = 0.0;

			if( otype === 'hsl' ) {
				delta = (this.lightness * amt);

				comps.lightness = this.lightness+delta;
				if(comps.lightness < 0) {
					remainder = (-1*comps.lightness);
					comps.lightness = 0.0;
				}
				else if(comps.lightness > 1.0) {
					remainder = (comps.lightness-1.0);
					comps.lightness = 1.0;
				}
			} 
			else {
				delta = (this.brightness * amt);

				comps.brightness = this.brightness+delta;
				if(comps.brightness < 0) {
					remainder = (-1*comps.brightness);
					comps.brightness = 0.0;
				}
				else if(comps.brightness > 1.0) {
					remainder = (comps.brightness-1.0);
					comps.brightness = 1.0;
				}
			}

			comps.saturation = (this.saturation+(-1*remainder) < 0)
					? 0.0
				: (this.saturation+(-1*remainder) > 1.0)
					? 1.0
					: this.saturation+(-1*remainder);

			return [ otype, comps ];
		},

		darken: function(amt, isNew) {
			amt *= 1.2;

			isNew = (isNew == undefined) ? false : isNew;

			var output = this._adjust(-amt);
			if( !isNew ) {
				this.hue 		= output[1].hue;
				this.saturation	= output[1].saturation;
				if( output[1].brightness != undefined ) {
					this.brightness = output[1].brightness;
				}
				else {
					this.lightness = output[1].lightness;
				}
				this.type = output[0];
				return this;
			}
			else {
				var _temp = new Color( output[1] );
				_temp = _temp.convert( output[0] );
				return _temp;
			}
		},

		lighten: function(amt, isNew) {
			amt *= 2.0;
			isNew = (isNew == undefined) ? false : isNew;
		...
		lerp: function( arg0, arg1, arg2 ) {
			...
			if(typeof arg1 === 'number') {
				...
				var acomps = arg0.getComponents();
				for( var i=0; i<comps.length; i++ ) {
					comps[i] = Calculation.lerp( comps[i],	acomps[i], arg1);
				}
			}
			else {
				var acomps = arg0.getComponents();
				var bcomps = arg1.getComponents();
				var comps = new Array( acomps.length );
				for( var i=0; i<acomps.length; i++ ) {
					comps[i] = Calculation.lerp( acomps[i],	bcomps[i], arg2);
				}
			}

			return new Color( otype, comps );
		},
